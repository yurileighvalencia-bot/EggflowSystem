...
@php
    $icons = [
        ...
        'egg' => '<path stroke-linecap="round" stroke-linejoin="round" d="M12 21c-4.418 0-8-4.03-8-9 0-4.97 3.582-9 8-9s8 4.03 8 9c0 4.97-3.582 9-8 9z" />',
...
<div {{ $attributes->merge(['class' => 'flex flex-col items-center justify-center py-12 px-4']) }}>
    ...
    @if($action)
        @if($actionUrl)
            <a href="{{ $actionUrl }}" class="inline-flex items-center gap-2 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white text-sm font-medium rounded-lg transition-colors">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                    {!! $actionIcons[$actionIcon] ?? $actionIcons['plus'] !!}
                </svg>
                {{ $action }}
            </a>
        @else
            <button type="button" {{ $attributes->whereStartsWith('wire:') }} class="inline-flex items-center gap-2 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white text-sm font-medium rounded-lg transition-colors">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                    {!! $actionIcons[$actionIcon] ?? $actionIcons['plus'] !!}
                </svg>
                {{ $action }}
