            ...
            <div class="flex-1 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col">
                ...
                <div class="p-4 border-b border-gray-200 dark:border-gray-700">
                    ...
                    @if($selectedCustomer)
                        ...
                        <div class="flex items-center justify-between bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-lg px-3 py-2">
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    ...
                                    @if($selectedCustomer->phone)
                                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ $selectedCustomer->phone }}</p>
                                    @endif
                                </div>
                            </div>
                    ...
                    @else
                        ...
                        <div class="relative" x-data="{ open: @entangle('showCustomerDropdown') }">
                            <div class="relative">
                                <input 
                                    ...
                                    class="w-full pl-10 pr-4 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white focus:ring-amber-500 focus:border-amber-500"
                                    @focus="if($wire.customerSearch.length >= 2) open = true"
                                >
                            ...
                            @if($showCustomerDropdown && !$showQuickAddCustomer)
                                <div class="absolute z-10 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                                    ...
                                    @empty
                                        <div class="px-4 py-3 text-center">
                                            ...
                                            <button 
                                                wire:click="showQuickAdd"
                                                class="mt-2 text-sm text-amber-600 dark:text-amber-400 hover:underline flex items-center justify-center gap-1 mx-auto"
                                            >
                            ...
                            @if($showQuickAddCustomer)
                                <div class="absolute z-10 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg p-4">
                                    <h4 class="text-sm font-semibold text-gray-900 dark:text-white mb-3">Quick Add Customer</h4>
                                    <div class="space-y-3">
                                        <div>
                                            ...
                                            <input 
                                                ...
                                                class="w-full text-sm border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white px-3 py-2"
                                        ...
                                        <div>
                                            <label class="block text-xs text-gray-600 dark:text-gray-400 mb-1">Phone (Optional)</label>
                                            <input 
                                                type="text" 
                                        ...
                                        <div class="flex gap-2">
                                            <button 
                                                wire:click="$set('showQuickAddCustomer', false)"
                                                class="flex-1 py-2 text-sm border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700"
                                            >
                                                Cancel
                                            </button>
                                            <button 
                                                ...
                                                class="flex-1 py-2 text-sm bg-amber-500 hover:bg-amber-600 text-white rounded-lg disabled:opacity-50"
                                            >
                                                <span wire:loading.remove wire:target="createQuickCustomer">Add & Select</span>
                                                <span wire:loading wire:target="createQuickCustomer">Adding...</span>
                                            </button>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Leave empty for walk-in sale</p>
                    @endif
                </div>
        ...
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" wire:click.self="$set('showCheckoutModal', false)">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full mx-4 p-6">
                ...
                <div class="space-y-4">
                    {{-- Customer Info --}}
                    @if($selectedCustomer)
                        <div class="flex items-center gap-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-lg px-4 py-2">
                            <svg class="w-5 h-5 text-amber-600 dark:text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                            </svg>
                            <div>
                                <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $selectedCustomer->name }}</p>
                                <p class="text-xs text-gray-500 dark:text-gray-400">{{ $selectedCustomer->phone ?? 'Registered Customer' }}</p>
                            </div>
                        </div>
                    @else
                        <div class="flex items-center gap-3 bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600 rounded-lg px-4 py-2">
                            <svg class="w-5 h-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                            </svg>
                            <p class="text-sm text-gray-500 dark:text-gray-400">Walk-in Customer</p>
                        </div>
                    @endif
                    {{-- Total --}}
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-sm w-full mx-4 p-6">
                ...
                <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 mb-4">
                    ...
                    @if($lastSale->customer)
                        <div class="flex items-center gap-2 pb-3 mb-3 border-b border-gray-200 dark:border-gray-600">
                            <svg class="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                            </svg>
                            <span class="text-sm text-gray-700 dark:text-gray-300">{{ $lastSale->customer->name }}</span>
                        </div>
                    @endif
